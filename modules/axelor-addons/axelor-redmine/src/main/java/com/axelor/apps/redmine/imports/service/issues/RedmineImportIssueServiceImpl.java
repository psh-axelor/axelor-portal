...
public class RedmineImportIssueServiceImpl extends RedmineImportService
  ...
  public void setTeamTaskFields(TeamTask teamTask, Issue redmineIssue) {
    ...
    teamTask.setAssignedTo(assignedTo != null ? assignedTo : project.getAssignedTo());

    teamTask.setProgressSelect(redmineIssue.getDoneRatio());

    Float estimatedHours = redmineIssue.getEstimatedHours();
    teamTask.setBudgetedTime(
        estimatedHours != null ? BigDecimal.valueOf(estimatedHours) : BigDecimal.ZERO);
    ...
    teamTask.setAccountedForMaintenance(value != null ? (value.equals("1") ? true : false) : false);
